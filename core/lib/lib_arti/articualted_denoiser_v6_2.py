# DEBUG version
# ! from v6: fix the sym bug, and adding sym sync
import torch
...
class GraphDenoiseConvV62(torch.nn.Module):
    # A large upgrade
    def __init__(
        self,
        K,  #
        ...
        assert not self.E_guidance_flag
        ...
        if self.sym_sync:
            self.e_wide_fc_symsync = nn.Linear(e_wide_c, self.e_conv_dims[-1])
    ...
    def forward(self, V_noisy, E_noisy, t, V_mask=None):
        ...
        for i, v_in_c in enumerate(self.V_dims):
            ...
            cur += v_in_c
        _edge_f_d = self.e_in_layers[0](E_dir)
        _edge_f_s = torch.zeros_like(_edge_f_d)
        cur = 3
        for i, e_in_c in enumerate(self.E_s_dims):
            e_input = E_sym[..., cur : cur + e_in_c]
            _edge_f_s = _edge_f_s + self.e_in_layers[i + 1](e_input)
        edge_f = _edge_f_s + _edge_f_d
        # node_f: B,K,F; edge_f: B,K^2,F
        edge_f_sym_list = [_edge_f_s]
        node_f_list, edge_f_list = [node_f], [edge_f] # now edge_f is dir
        
        for i in range(len(self.layers)):
            node_f, edge_f = self.layers[i](
                node_f, edge_f, emb_p, emb_t, e_agg_mask=None, v_agg_mask=V_mask
            )
            node_f_list.append(node_f)
            edge_f_list.append(edge_f)
            if self.sym_sync:
                edge_f = edge_f.reshape(B, K, K, -1)
                edge_f = 0.5 * (edge_f + edge_f.permute(0,2,1,3))
                edge_f = edge_f.reshape(B, K**2, -1)
                edge_f_sym_list.append(edge_f)
        ...
        if self.dir_handling:
            if self.sym_sync:
                edge_f_sym = torch.cat(edge_f_sym_list, -1)
                edge_f_sym = self.e_wide_fc_symsync(edge_f_sym).reshape(B, K, K, -1)
            else:
        ...
        for i in range(len(self.E_s_dims)):
            e_pred.append(
                self.e_out_layers[i + 1](
                    torch.cat([edge_f_sym, E_noisy[..., cur : cur + self.E_s_dims[i]]], -1)
                )
            )
            cur += self.E_s_dims[i]
