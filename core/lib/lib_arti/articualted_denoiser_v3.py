# V3: from v2, use number of nodes count
import torch
...
class GraphDenoiseConvV3(torch.nn.Module):
    def __init__(
        self,
        K,  #
        M,  # time steps
        ...
        self.p_pe_dim, self.t_pe_dim, self.n_pe_dim = p_pe_dim, t_pe_dim, n_pe_dim
        self.register_buffer("t_pe", self.sinusoidal_embedding(M, self.t_pe_dim))
        self.register_buffer("n_pe", self.sinusoidal_embedding(K, self.n_pe_dim))
        if self.p_pe_dim > 0:
        ...
        for c_out in node_conv_dims:
            self.te_layers.append(
                nn.Sequential(
                    nn.Linear(self.t_pe_dim + self.n_pe_dim, self.t_pe_dim),
                    nn.LeakyReLU(),
            ...
            self.pe_layers.append(
                nn.Sequential(
                    nn.Linear(self.p_pe_dim + self.n_pe_dim, self.p_pe_dim),
                    nn.LeakyReLU(),
            ...
            self.v_self_layers0.append(
                nn.Sequential(
                    nn.Linear(c_in + self.p_pe_dim + self.n_pe_dim, c_out // 2),
            ...
            self.v_self_layers1.append(
                nn.Sequential(
                    nn.Linear(c_out // 2 + self.t_pe_dim + self.n_pe_dim, c_out // 2),
                    BN_lastdim(c_out // 2) if use_bn else nn.Identity(),
                    nn.LeakyReLU(),
                )
            )
            ...
            self.e_dir_conv_layers.append(
                nn.Sequential(
                    nn.Linear(self.e_d_dim + self.n_pe_dim, c_out // 2),
                    BN_lastdim(c_out // 2) if use_bn else nn.Identity(),
                    nn.LeakyReLU(),
                )
            )
            self.e_sym_conv_layers.append(
                nn.Sequential(
                    nn.Linear(self.e_s_dim + self.n_pe_dim, c_out // 2),
                    BN_lastdim(c_out // 2) if use_bn else nn.Identity(),
                    nn.LeakyReLU(),
                )
            )
            ...
            self.e_conv_layers.append(
                nn.Sequential(
                    nn.Linear(e_conv_in + self.n_pe_dim, c_out),
            ...
            self.v_self_layers2.append(
                nn.Sequential(
                    nn.Linear(c_out + c_in + self.n_pe_dim, c_out),
                    BN_lastdim(c_out) if use_bn else nn.Identity(),
                    nn.LeakyReLU(),
                )
            )
        ...
        if self.use_wide:
            self.wide_fc = nn.Linear(out_cnt + self.n_pe_dim, c_out)

        self.node_mlp = NaiveMLP(
            c_in + self.n_pe_dim, self.v_dim, node_pred_head_dims, use_bn=use_bn
        )
        self.edge_sym_mlp = NaiveMLP(
            c_in * 2 + edge_wide_cnt + 10 + self.n_pe_dim,
            self.e_s_dim,
            edge_sym_head_dims,
            use_bn=use_bn,
        )
        self.edge_dir_mlp = NaiveMLP(
            c_in * 2 + edge_wide_cnt + 3 + self.n_pe_dim,
            self.e_d_dim,
            edge_dir_head_dims,
            use_bn=use_bn,
        )
    ...
    def forward(self, V_noisy, E_noisy, V_mask, t):
        ...
        # The number of nodes embedding is everywhere
        v_cnt = V_mask.sum(dim=1).long().squeeze(-1)
        emb_N = self.n_pe[v_cnt - 1].clone()  # B,PE
        emb_N_padV = emb_N[:, None, :].expand(-1, K, -1)
        emb_N_padE_un = emb_N[:, None, :].expand(-1, (K * (K - 1)) // 2, -1)
        emb_N_padE = emb_N[:, None, :].expand(-1, K**2, -1)
        ...
        for i in range(len(self.v_self_layers0)):
            # process self
            _pe_t = self.te_layers[i](torch.cat([emb_t, emb_N], -1))
            _pe_t = _pe_t.unsqueeze(1).expand(-1, K, -1)
            if self.p_pe_dim > 0:
                _pe_p = self.pe_layers[i](torch.cat([emb_p, emb_N_padV], -1))
                _node_f = torch.cat([node_f, _pe_p], -1)
            else:
                _node_f = node_f
            _node_f = self.v_self_layers0[i](torch.cat([_node_f, emb_N_padV], -1))
            _node_f = self.v_self_layers1[i](torch.cat([_node_f, _pe_t, emb_N_padV], -1))
            # process edges
            # During aggregation, the row is dst, each col is a neighbor
            e_f_sym = self.e_sym_conv_layers[i](torch.cat([E_sym, emb_N_padE], -1))
            e_f_dir = self.e_dir_conv_layers[i](torch.cat([E_dir, emb_N_padE], -1))
            ...
            e_f = self.e_conv_layers[i](torch.cat([e_f, emb_N_padE], -1))
            ...
            node_f = self.v_self_layers2[i](torch.cat([node_f, agg_f, emb_N_padV], -1))
            if self.use_wide:
                node_f_list.append(node_f)
        if self.use_wide:
            node_f = torch.cat(node_f_list, -1)
            node_f = self.wide_fc(torch.cat([node_f, emb_N_padV], -1))
        edge_f_list = torch.cat(edge_f_list, -1)  # B,K,K,ALL EF

        v_pred = self.node_mlp(torch.cat([node_f, emb_N_padV], -1))
        ...
        e_f_sym = torch.cat([e_f_sym, edge_f_sym, emb_N_padE_un], -1)
        e_f_dir = torch.cat([e_f_dir, edge_f_dir, emb_N_padE_un], -1)
