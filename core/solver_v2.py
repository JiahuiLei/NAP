...
Because on different dataset, sometimes the iteration is the key fact, not the epoch, update

the lr decay, the model viz, the model eval, save is controlled by the iteration
...
class Solver_v2(object):
    def __init__(self, cfg, model, datasets_dict, logger):
        ...
        for mode in cfg["modes"]:  # prepare dataloader
            ...
            training_drop_last = cfg_with_default(cfg, ["training", "drop_last"], True)
            try:
                sampler = datasets_dict[mode].get_sampler()
            except:
                sampler = None
            self.dataloader_dict[mode] = DataLoader(
                datasets_dict[mode],
                batch_size=bs,
                shuffle=shuffle_dataset and sampler is None,
                num_workers=n_workers,
                pin_memory=cfg["dataset"]["pin_mem"],
                drop_last=mode == "train" and training_drop_last,
